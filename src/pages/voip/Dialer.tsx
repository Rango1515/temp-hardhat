...
export default function Dialer() {
  ...
  const requestNextLead = async () => {
    ...
    if (result.error) {
      ...
      setCurrentLead(result.data.lead);
      // Reset timer and start session immediately on lead assignment
  ...
  const handleTextNowOpen = () => {
    // Timer now starts on lead assignment, not on TextNow open
    // This callback is kept for compatibility but no longer starts the timer
    if (!sessionStartTime && currentLead) {
      setSessionStartTime(Date.now());
      setHasStartedSession(true);
    }
  };
