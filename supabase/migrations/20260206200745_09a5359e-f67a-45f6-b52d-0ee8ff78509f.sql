...
CREATE OR REPLACE FUNCTION public.assign_next_lead(p_worker_id integer, p_category varchar DEFAULT NULL)
 RETURNS TABLE(out_lead_id integer, out_name text, out_phone text, out_email text, out_website text, out_contact_name text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
    ...
    -- Find and lock an available lead atomically (with optional category filter)
