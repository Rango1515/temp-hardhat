-- Fix the assign_next_lead function - alias id as lead_id to match return type
CREATE OR REPLACE FUNCTION public.assign_next_lead(p_worker_id integer)
RETURNS TABLE(lead_id integer, name text, phone text, email text, website text)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- Return the lead details with proper column alias
    RETURN QUERY
    SELECT l.id AS lead_id, l.name, l.phone, l.email, l.website
    FROM voip_leads l
